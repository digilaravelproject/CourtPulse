    ...
    <div class="bg-navy2 rounded-3xl border border-white/5 shadow-2xl p-8 mb-10">
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-white/40 text-xs font-bold uppercase tracking-widest mt-2">Toggle global menu visibility and
                    modify navigation labels</p>
            </div>
    ...
    <div class="space-y-4">
            ...
            <div
                ...
                <div class="flex flex-wrap items-end gap-6 w-full lg:w-auto justify-between lg:justify-end">
                    ...
                    <div class="flex flex-col gap-2">
                        <label
                            class="text-[0.6rem] font-black text-white/40 uppercase tracking-widest ml-1">Visibility</label>
                        <div class="flex items-center bg-navy border border-white/10 rounded-xl p-1 shadow-inner">
                            <button type="button" onclick="updateVisibilityUI(this, 1)"
                            ...
                            <button type="button" onclick="updateVisibilityUI(this, 0)"
                            ...
                            <input type="hidden" name="is_visible" id="visible-{{ $menu->id }}"
                                value="{{ $menu->is_visible ? 1 : 0 }}">
                        </div>
                    </div>
        ...
        @empty
            <div class="bg-navy2 rounded-3xl border border-white/5 shadow-2xl p-20 text-center">
                <div
                    class="w-20 h-20 rounded-3xl bg-white/5 border border-white/10 flex items-center justify-center text-white/20 text-3xl mx-auto mb-4">
...
@push('scripts')
    <script>
        function updateVisibilityUI(btn, value) {
            ...
            buttons.forEach(b => {
                b.className =
                    'px-5 py-2.5 rounded-lg text-[0.6rem] font-black uppercase tracking-widest transition-all text-white/30 hover:text-white/60 border border-transparent';
            });

            if (value === 1) {
                btn.className =
                    'px-5 py-2.5 rounded-lg text-[0.6rem] font-black uppercase tracking-widest transition-all bg-green-500/20 text-green-400 border border-green-500/30 shadow-[0_0_10px_rgba(34,197,94,0.15)]';
            } else {
                btn.className =
                    'px-5 py-2.5 rounded-lg text-[0.6rem] font-black uppercase tracking-widest transition-all bg-red-500/20 text-red-400 border border-red-500/30 shadow-[0_0_10px_rgba(239,68,68,0.15)]';
            }
        }

        async function saveMenu(btn, menuId, url) {
            const label = document.getElementById(`label-${menuId}`).value;
            const is_visible = document.getElementById(`visible-${menuId}`).value;
