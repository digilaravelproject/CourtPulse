            ...
            <template x-for="adv in advocates" :key="adv.id">
                <div
                    ...
                    <div class="flex items-center gap-1.5 text-xs text-muted mb-3">
                        <i class="bi bi-clock-history shrink-0" style="color:#D4AF37"></i>
                        <span x-text="(adv.advocate_profile?.experience_years || 0) + ' yrs experience'"></span>
                    </div>

                    {{-- Practice area tags --}}
                    <div class="flex flex-wrap gap-1 mb-4" x-show="adv.advocate_profile?.practice_areas?.length">
                    ...
                    <div class="mt-auto flex flex-col gap-2 pt-3 border-t border-border">
                        <div class="flex items-center gap-2 w-full">
                            ...
                            {{-- Connection States --}}
                            <template x-if="adv.connection_status === 'none'">
                                <button @click="sendConnection(adv.id)"
                            ...
                            <template x-if="adv.connection_status === 'sent'">
                                <button disabled
                            ...
                            <template x-if="adv.connection_status === 'received'">
                                <button disabled
                                    class="flex-1 py-2 rounded-lg text-xs font-bold transition-all flex justify-center items-center gap-1 bg-gray-100 text-gray-500 border border-gray-200 cursor-not-allowed">
                            ...
                            <template x-if="adv.connection_status === 'connected'">
                                <button disabled
                    ...
                    async sendConnection(userId) {
                        try {
                            ...
                            if (res.ok) {
                                ...
                                const adv = this.advocates.find(a => a.id === userId);
                                if (adv) adv.connection_status = 'sent';
                            }
                        } catch (e) {
                            console.error(e);
                        }
