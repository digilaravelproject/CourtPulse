...
<body class="font-sans text-ink min-h-screen" style="font-weight: 400;" x-data="guestLayout()">
    ...
    <div class="fixed top-4 right-4 z-9999 flex flex-col gap-2 w-72 pointer-events-none">
        <template x-for="t in toasts" :key="t.id">
            <div x-show="t.show" x-transition:enter="transition ease-out duration-300"
                ...
                x-transition:leave-end="opacity-0" :class="t.type === 'ok' ? 'bg-white border-green-200 text-green-700' :
                ...
                <i :class="t.type === 'ok' ? 'bi-check-circle-fill text-green-500' :
                    'bi-exclamation-circle-fill text-red-500'" class="bi shrink-0"></i>
    ...
    <nav class="sticky top-0 z-100 bg-white border-b border-border">
        <div class="max-w-[1500px] mx-auto px-4 h-14 flex items-center gap-2">
            ...
            <a href="{{ route('guest.dashboard') }}"
                class="flex items-center gap-2 shrink-0 font-display font-bold text-xl text-ink no-underline">
    ...
    <footer class="border-t border-border bg-white mt-10 py-5 hidden md:block">
        <div class="max-w-[1500px] mx-auto px-4 flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="text-xs text-muted">© {{ date('Y') }} DockIt. All rights reserved.</div>
        </div>
    </footer>
