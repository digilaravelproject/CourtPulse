...
<head>
    ...
    <style>
        ...
        /* Super admin sidebar accent — purple tint on active */
        .nav-super-active {
            position: relative;
        ...
        <nav class="flex-1 overflow-y-auto py-3 space-y-0.5">
            @php
                ...
                if (!function_exists('superNavLink')) {
                    function superNavLink($route, $icon, $label)
                    {
                        $active = request()->routeIs($route . '*');
                        $cls = $active
                            ? 'nav-super-active'
            ...
            {{-- Admin section --}}
            <div class="px-5 pt-3 pb-1 font-mono text-[0.5rem] tracking-[2.5px] uppercase text-white/20">Admin</div>
            ...
            <div class="mx-4 my-3 border-t border-purple-500/20"></div>
            <div class="px-5 pb-1 font-mono text-[0.5rem] tracking-[2.5px] uppercase" style="color:#a855f7;opacity:0.7">
                Super Admin</div>
            {!! superNavLink('super.roles', 'bi-shield-lock', 'Roles & Permissions') !!}
            {!! superNavLink('super.permissions', 'bi-key', 'Permissions') !!}
            {!! superNavLink('super.activity', 'bi-clock-history', 'Activity Logs') !!}
        ...
        <div class="flex-shrink-0 px-5 py-4 border-t border-white/[0.06]">
            <div class="flex items-center gap-3">
                ...
                <div class="overflow-hidden flex-1 min-w-0">
                    ...
                    <div class="font-mono text-[0.55rem] tracking-wide uppercase" style="color:#a855f7">Super Admin
                    </div>
                </div>
    ...
    <header
        ...
        <div class="ml-auto flex items-center gap-2">
            ...
            <a href="{{ route('admin.documents') }}"
                class="relative w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:border-gold hover:text-gold transition-all">
            ...
            <a href="{{ route('admin.users') }}?status=pending"
                class="relative w-9 h-9 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-500 hover:border-red-400 hover:text-red-500 transition-all">
            ...
            {{-- Super admin badge --}}
            <div class="w-9 h-9 rounded-lg border flex items-center justify-center font-bold text-[0.82rem]"
                style="background:rgba(168,85,247,.12);border-color:rgba(168,85,247,.3);color:#a855f7">
                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
            </div>
        </div>
    </header>
    ...
    <main class="lg:ml-64 pt-[58px] min-h-screen">
        <div class="p-5 lg:p-6">
            @foreach (['success' => 'green', 'error' => 'red', 'info' => 'amber'] as $key => $color)
                @if (session($key))
                    <div
                        class="flex items-center gap-3 mb-4 px-4 py-3 rounded-xl bg-{{ $color }}-50 border border-{{ $color }}-200 text-{{ $color }}-700 text-sm font-medium">
                        <i
