            ...
            <div>
                ...
                <div class="text-[10px] text-blue font-bold tracking-[0.2em] uppercase leading-none">
                    @if(auth()->user()->role === 'advocate')
                        Advocate
                    @elseif(auth()->user()->sub_role === 'ca')
                        CA
                    @elseif(auth()->user()->sub_role === 'cs')
                        CS
                    @else
                        {{ auth()->user()->sub_role ?? auth()->user()->role }}
                    @endif
                </div>
            </div>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-8">
            ...
            <div class="space-y-1">
                ...
                <a href="{{ route('professional.dashboard') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.dashboard') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-th-large w-5"></i>
                    <span class="text-sm font-semibold">Dashboard</span>
                </a>

                <a href="{{ route('professional.pending.requests') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.pending.requests') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
                ...
                <a href="{{ route('professional.connections') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.connections') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-users w-5"></i>
                    <span class="text-sm font-semibold">My Connections</span>
                </a>

                <a href="{{ route('professional.search.clerks') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.search.clerks') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-search w-5"></i>
                    <span class="text-sm font-semibold">Search Clerks</span>
                </a>

                <a href="{{ route('professional.feedback') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.feedback') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
            ...
            <div class="space-y-1">
                ...
                <a href="{{ route('professional.profile') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.profile') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
                    <i class="fas fa-user-circle w-5"></i>
                    <span class="text-sm font-semibold">My Profile</span>
                </a>

                {{-- <a href="{{ route('professional.settings') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('professional.settings') ? 'bg-blue/10 text-blue border-l-4 border-blue' : 'text-white/50 hover:bg-white/5 hover:text-white' }}">
        ...
        <div class="p-4 border-t border-white/5 bg-navy/20">
            <div class="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/5">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-[10px] text-blue font-medium uppercase tracking-wider">{{ auth()->user()->sub_role ?? auth()->user()->role }}</div>
                </div>
    ...
    <div class="lg:ml-[280px] min-h-screen flex flex-col transition-all duration-300">
        ...
        <header class="sticky top-0 z-40 bg-navy/80 backdrop-blur-xl border-b border-white/5 px-4 lg:px-8 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('professional.profile') }}" class="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-blue hover:border-blue/50 transition-all">
