...
@section('content')
    ...
    <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div class="space-y-1">
            <h2 class="text-4xl font-black text-white uppercase tracking-tighter leading-tight">Professional <span
                    class="text-blue-500">Dashboard</span></h2>
            <p class="text-xs font-bold text-white/30 uppercase tracking-[0.2em]">Overview of your performance & network
                status</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        {{-- Pending Requests --}}
        <a href="{{ route('professional.pending.requests') }}"
        ...
        <div
            ...
            <div class="relative z-10">
                <div class="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-4">Connected Clerks</div>
                <div class="flex items-baseline gap-2">
        ...
        <div class="relative z-10">
            <div class="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-4">Feedback Rating</div>
            <div class="flex items-baseline gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 space-y-8">
                ...
                <div class="relative z-10 flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div class="flex-1 text-center md:text-left">
                        ...
                        <p class="text-base font-medium text-white/50 leading-relaxed max-w-xl italic">
                            "{{ $profile->bio ?? 'Your profile is incomplete. Update your information to improve your visibility and connection requests.' }}"
                        </p>
                    </div>
                    ...
                    <div class="flex flex-col gap-4 min-w-[180px] w-full md:w-auto">
                        ...
                        <a href="{{ route('professional.settings') }}"
                            class="w-full py-5 rounded-2xl bg-white/5 border border-white/10 text-white text-xs font-black uppercase tracking-widest text-center hover:bg-white/10 transition-all duration-300">
                            Account Settings
                        </a>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                {{-- Search Clerks --}}
                <a href="{{ route('professional.search.clerks') }}"
                    ...
                    <p class="text-[10px] font-bold text-white/30 uppercase tracking-widest">Find support staff</p>
                </a>

                {{-- Search Advocates --}}
                <a href="{{ route('professional.search.advocates') }}"
                    ...
                    <p class="text-[10px] font-bold text-white/30 uppercase tracking-widest">Networking</p>
                </a>

                {{-- Search Courts --}}
                <a href="{{ route('professional.search.courts') }}"
                    class="group p-8 rounded-[2.5rem] bg-white/5 border border-white/5 hover:border-fuchsia-500/30 transition-all duration-500 backdrop-blur-xl">
                    <div
                        class="h-16 w-16 rounded-2xl bg-fuchsia-500/10 border border-fuchsia-500/10 flex items-center justify-center text-fuchsia-400 mb-8 group-hover:scale-110 group-hover:bg-fuchsia-500/20 transition-all duration-500">
                        <i class="fas fa-landmark text-2xl"></i>
                    </div>
                    <h4 class="text-base font-black text-white uppercase tracking-widest mb-2">Courts</h4>
                    <p class="text-[10px] font-bold text-white/30 uppercase tracking-widest">Locate court centers</p>
                </a>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 space-y-8">
            ...
            <div class="p-10 rounded-[3rem] bg-white/5 border border-white/10 backdrop-blur-2xl">
                ...
                <div class="space-y-5">
                    <a href="{{ route('professional.connections') }}"
                        class="flex items-center justify-between p-5 rounded-2xl bg-white/2 border border-white/5 hover:bg-white/5 hover:border-blue-500/20 transition-all group">
                        <div class="flex items-center gap-5">
                            <div
                                class="h-12 w-12 rounded-xl bg-blue-500/10 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform">
                                <i class="fas fa-users"></i>
                            </div>
                            <span class="text-xs font-black text-white/70 uppercase tracking-widest">My Network</span>
                        </div>
                        <i
                            class="fas fa-chevron-right text-[10px] text-white/20 group-hover:text-blue-500 group-hover:translate-x-1 transition-all"></i>
                    </a>

                    <a href="{{ route('professional.feedback') }}"
                        ...
                        <div class="flex items-center gap-5">
                            ...
                            <span class="text-xs font-black text-white/70 uppercase tracking-widest">Feedback Feed</span>
                        </div>
                        <i
                    ...
                    {{-- <div class="pt-8 mt-4 border-t border-white/5">
                        <div
                            ...
                            <div class="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-4">Tips</div>
                ...
                <div class="space-y-8">
                    <div>
                        <div class="flex justify-between mb-3">
                            <span class="text-[10px] font-black text-white/40 uppercase tracking-widest">Profile
                                Completion</span>
                            <span
                                class="text-[10px] font-black text-blue-400 uppercase tracking-widest">{{ $profile ? '100%' : '20%' }}</span>
                        </div>
                        <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
                            <div class="h-full bg-blue-500 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.5)]"
                                style="width: {{ $profile ? '100%' : '20%' }}"></div>
                        </div>
                    </div>

                    <div>
                        <div class="flex justify-between mb-3">
                            <span class="text-[10px] font-black text-white/40 uppercase tracking-widest">Network
                                Growth</span>
                            <span
                                class="text-[10px] font-black text-indigo-400 uppercase tracking-widest">{{ min(($totalConnections ?? 0) * 5, 100) }}%</span>
                        </div>
                        <div class="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
                            <div class="h-full bg-indigo-500 rounded-full shadow-[0_0_15px_rgba(99,102,241,0.5)]"
                                style="width: {{ min(($totalConnections ?? 0) * 5, 100) }}%"></div>
                        </div>
                    </div>
                </div>
            </div> --}}
